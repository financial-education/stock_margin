...
The only difference between a short and long position is how you calculate =Equity=.  For a long position:

$$Equity = Market\ Value - Loan$$

where $Loan = Purchase\ Price * Number\ of\ Shares * (1 - Initial\ Margin)$.  

For a short position we have:

$$Equity = Cash\ from\ Sale + Margin\ Deposit - Market\ Value$$

where $Margin\ Deposit = Cash\ from\ Sale * Initial\ Margin$. 
